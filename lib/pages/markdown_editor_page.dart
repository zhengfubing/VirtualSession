...
enum PromptDetailType { role, scene, world, systemPrompt }
...
class _MarkdownEditorPageState extends State<MarkdownEditorPage> {
  ...
  Future<void> _loadContent() async {
    ...
    switch (widget.type) {
      case PromptDetailType.role:
        content = await _fileStorage.readRole(widget.prompt.fileName);
        break;
      case PromptDetailType.scene:
        content = await _fileStorage.readScene(widget.prompt.fileName);
        break;
      case PromptDetailType.world:
        content = await _fileStorage.readWorld(widget.prompt.fileName);
        break;
      case PromptDetailType.systemPrompt:
        content = await _fileStorage.readSystemPrompt(widget.prompt.fileName);
        break;
    }
  ...
  Future<void> _saveContent() async {
    ...
    switch (widget.type) {
      case PromptDetailType.role:
        await _fileStorage.saveRole(widget.prompt.fileName, content);
        break;
      case PromptDetailType.scene:
        await _fileStorage.saveScene(widget.prompt.fileName, content);
        break;
      case PromptDetailType.world:
        await _fileStorage.saveWorld(widget.prompt.fileName, content);
        break;
      case PromptDetailType.systemPrompt:
        await _fileStorage.saveSystemPrompt(widget.prompt.fileName, content);
        break;
    }
    _originalContent = content;
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('$_typeName保存成功')),
      );
    }
  }
  ...
  Future<void> _share() async {
    ...
    final file = File('${dir.path}/${widget.prompt.fileName}.md');
    await file.writeAsString(content);
    await Share.shareXFiles(
      [XFile(file.path)],
      subject: widget.prompt.fileName,
    );
  }
