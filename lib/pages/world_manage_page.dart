...
class WorldManagePage extends StatefulWidget {
  const WorldManagePage({super.key, this.embedded = false});

  final bool embedded;

  @override
  State<WorldManagePage> createState() => WorldManagePageState();
}

class WorldManagePageState extends State<WorldManagePage> {
  ...
  static const _config = PromptManagePageConfig.world;
  ...
  Future<void> _loadItems() async {
    ...
    final items = await _promptDao.getAllWorlds();
    if (mounted) {
      setState(() {
        _items = items;
        _isLoading = false;
  ...
  Future<void> _createWithName(String name) async {
    final exists = await _promptDao.worldExists(name);
    if (exists) {
      if (mounted) {
  ...
  Future<void> importFile() async {
    try {
      ...
      final exists = await _promptDao.worldExists(nameWithoutExt);
      if (exists) {
        if (mounted) {
          ScaffoldMessenger.of(
            context,
      ...
      await _fileStorage.saveWorld(nameWithoutExt, content);
      await _promptDao.createWorld(nameWithoutExt);
      _loadItems();
  ...
  Widget _buildAvatar(Prompt item) {
    ...
    return CircleAvatar(
      radius: 22,
      backgroundColor: _config.avatarBgColor ?? accent.withValues(alpha: 0.1),
      child: Text(
